...
if __name__ == "__main__":
    bufferA = RolloutStorage(19, 1, obs_space=np.ones((2, 2)), act_space=np.ones(1))
    for i in range(0, 19):
        bufferA.append(np.ones((2, 2))*i, np.ones(1)*i, np.ones(1) * i,
                       np.ones(1)*i, np.ones(1)*(i % 5 == 0), np.ones(1) * 100*i)
    bufferA.compute_returns(np.ones(1)*1900, np.ones(1)*0)
    print(bufferA.advantages)
    print("-----")
    print(bufferA.returns)
    print("-----")
